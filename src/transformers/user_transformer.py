...
from src.config.schemas import validate_transformed_data
from src.exceptions.custom_exceptions import TransformationError
from src.utils.validators import safe_get_nested, sanitize_email, sanitize_string
        ...
        try:
            ...
            if not is_valid:
                raise TransformationError(
                    f"Validation failed: {'; '.join(errors)}", source_data=raw_data
                )
            ...
            logger.error(
                f"Transformation failed: {e}", extra={"error_type": type(e).__name__}
            )
            raise TransformationError(
                f"Failed to transform user data: {e}", source_data=raw_data
            )
    ...
    def _extract_fields(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        transformed = {
            ...
            "last_name": sanitize_string(
                safe_get_nested(raw_data, "name", "last"), default="Unknown"
            ),
            "gender": sanitize_string(raw_data.get("gender"), default="other").lower(),
            "address": address,
            "email": sanitize_email(raw_data.get("email")),
            "username": sanitize_string(
                safe_get_nested(raw_data, "login", "username"), default=""
            ),
            "dob": sanitize_string(
                safe_get_nested(raw_data, "dob", "date"), default=""
            ),
            "registered_date": sanitize_string(
                safe_get_nested(raw_data, "registered", "date"), default=""
            ),
            "phone": sanitize_string(raw_data.get("phone"), default=""),
            "picture": sanitize_string(
                safe_get_nested(raw_data, "picture", "medium"), default=""
            ),
        }
